...
class Month_sales extends JPanel implements ActionListener {
	/**
	 * 
	 */
	...
	public Month_sales() { // 월별
		String[] title = { "월", "순이익", "총 판매금액", "총  구매금액" }; 
		...
		try {
			...
			Connection connect = DBConnManager.getConnection(); // DB연결
			Statement st = connect.createStatement();
			ResultSet rs = st.executeQuery("select * from sales_month"); // sales_month 테이블의 모든값을 가져온다.
			model = (DefaultTableModel) table.getModel();

			Object data[] = new Object[4];
			while (rs.next()) {
				data[0] = rs.getString(1); // 월
				data[1] = rs.getInt(2); // 월별 순이익
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == resetbtn) { // 새로고침 버튼
			try {
				...
				Statement st = connect.createStatement();
				ResultSet rs = st.executeQuery("select * from sales_month"); // sales_month 모든 값을 불러온다
