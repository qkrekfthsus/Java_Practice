...
public class DaycareFrame extends JFrame implements ActionListener {
	static String sqlSelectAll = "select name, tel,address,publicprivate,Facilitytype from daycarecenter;";
	static String sqlInsert = "insert daycarecenter(name, tel,address,publicprivate,Facilitytype) values(?, ?, ?,?,?);";
	static String sqlDelete = "delete from daycarecenter where name = ? AND tel=?;";
	...
	JTable table;
	JTextField nameText, phoneText, adText,puText;
	JButton insButton, delButton, searchButton, infobtn, personbtn;
	JPanel panel;
	JLabel status;

	
	DaycareFrame() {
		setTitle("어린이집 DB 관리(원장)");
		...
		try {
			...
			if (src == insButton) { 		// 삽입
				...
				if (num > 0) {
					...
					setStatus(num + "개의 어린이집 정보가 추가되었습니다.");
				}
				else
					setStatus("어린이집 정보가 추가되지 않았습니다.");
			}
				...
				if (row == -1) {
					...
					if (num > 0) {
						model.removeRow(row);
						setStatus(num + "개의 어린이집 정보가 삭제되었습니다.");
					}
					else
						setStatus("어린이집 정보가 삭제되지 않았습니다.");
